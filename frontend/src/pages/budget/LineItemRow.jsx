...
export default function LineItemRow({ item, onUpdate, onDelete }) {
  ...
  const showUnitPriceQty = item.item_name === '식대' || item.unit_price != null || item.quantity != null;
    ...
    <>
      <tr style={{ borderTop: '1px solid var(--border)' }}>
        <td style={{ padding: 6 }}>
          ...
          />
          {item.excluded_from_budget && (
            <span className="badge badge-neutral" style={{ display: 'block', marginTop: 2, fontSize: '0.65rem', width: 'fit-content' }}>예산 제외</span>
          )}
        </td>
        <td style={{ padding: 6 }}>
          <input
        ...
        <td style={{ padding: 6, fontWeight: 600, whiteSpace: 'nowrap' }}>{won(item.final_amount)}</td>
        <td style={{ padding: 6 }}>
        ...
        <td style={{ padding: 6, whiteSpace: 'nowrap' }}>
          <button className="btn-ghost" onClick={() => setShowDetail((v) => !v)}>{showDetail ? '접기' : '상세'}</button>
        </td>
        <td style={{ padding: 6 }}>
      ...
      {showDetail && (
        <tr>
          {/* width:0 + minWidth:100% 트릭 — 이 div가 없으면 안의 form-row(필드 여러 개, 각각 min-width 있음)가
              펼쳐진 채로 계산돼서 table-layout:auto가 표 전체 너비를 그만큼 넓혀버림(모바일에서 표가 옆으로
              길게 늘어나는 원인). 이 div로 감싸면 표는 원래 너비를 유지하고, 안의 필드들만 폭에 맞게 줄바꿈됨. */}
          <td colSpan={9} style={{ padding: '8px 6px 16px', background: 'var(--bg)', width: 0 }}>
